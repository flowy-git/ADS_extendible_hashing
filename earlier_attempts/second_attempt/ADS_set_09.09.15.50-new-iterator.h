...
public:
  ...
  Iterator() : set(nullptr), bucket_index(0), element_index(0), elem_ptr(nullptr) {}

  Iterator(const ADS_set* set, size_t bucket_index = 0, size_t element_index = 0) : set(set), bucket_index(bucket_index), element_index(element_index), elem_ptr(nullptr) {
    skip();
    //if (bucket_index < size && set->directory.buckets[bucket_index]->count == 0) {skip();}
  };
Iterator(const Iterator& other) : set(other.set), bucket_index(other.bucket_index), element_index(other.element_index), elem_ptr(other.elem_ptr) {}
  ...
  void skip() {
    ...
    while (bucket_index < size){
        ...
        //if(element_index >= set->directory.buckets[bucket_index]->count || visited[bucket_index]) {
            ...
            elem_ptr = &(set->directory.buckets[bucket_index]->elements[element_index]);
            return;
        }
    }
    elem_ptr = nullptr;
