...
public:
    ...
    Iterator(const ADS_set* set, size_t bucket_index = 0, size_t element_index = 0)
...
/*             for (size_t j{bucket_index}; j < size; j += (1 << set->directory.buckets[bucket_index]->local_depth)) {
    ...
    /* std::cerr << "Ending skip: bucket_index=" << bucket_index
              << ", element_index=" << element_index << std::endl; */
}
};
