...
void dump(std::ostream &o = std::cerr) const {
  ...
  void swap(ADS_set<Key, N> &lhs, ADS_set<Key, N> &rhs) {
    std::swap(lhs.current_size, rhs.current_size);
    std::swap(lhs.directory.global_depth, rhs.directory.global_depth);
    std::swap(lhs.directory.buckets, rhs.directory.buckets);
    }
};
...
typename ADS_set<Key, N>::const_iterator ADS_set<Key, N>::find(const key_type &key) const{
    ...
    // INTRODUCES LARGE OVERHEAD - GIVES EXPONENTIAL COMPLEXITY WITH GLOBAL DEPTH
    /* size_type size = 1 << directory.global_depth;
    for (size_t hash{0}; hash < size; ++hash){
        Bucket* bucket = directory.buckets[hash];
        for(size_t i{0}; i < bucket->count; ++i){
            if(key_equal{}(bucket->elements[i], key)){
                //std::cerr << "////////////\n///////////Found element: bucket_index=" << hash << " element_index=" << i << std::endl;
...
public:
    ...
    Iterator(const ADS_set* set, size_t bucket_index = 0, size_t element_index = 0, bool noskip = false)// bool skip_other_buckets = false)
        ...
        if(skip_other_buckets){
            for(size_t i{0}; i < size; ++i){
                size_t local_depth = set->directory.buckets[i]->local_depth;
                for(size_t j{i + (1 << local_depth)}; j < size; j += (1 << local_depth)){
                    if(j == bucket_index){
                        visited[i] = true;
                    }
                    else{
                        if(set->directory.buckets[i] == set->directory.buckets[j])
                            visited[j] = true;
                    }
                }
            }
    ...
    reference operator*() const {
        ////////////////
        //std::cerr << "Accessing element: bucket_index=" << bucket_index << ", element_index=" << element_index << ", element=" << set->directory.buckets[bucket_index]->elements[element_index] << std::endl;
        ///////////////
    ...
    Iterator& operator++() {
        if(isAtEnd()){
            //std::cerr << "Increment blocked: already at end\n";
            return *this;
        }
 //std::cerr << "Increment: bucket_index=" << bucket_index << ", element_index=" << element_index << std::endl;
        ++element_index;
        skip();
        if(isAtEnd()){
            bucket_index = 1 << set->directory.global_depth;
            element_index = 0;
            //std::cerr << "/////    /////Iterator explicitly set to end\n";
            // *this = set->end();
        }
    //std::cerr << "Post-Increment: bucket_index=" << bucket_index << ", element_index=" << element_index << std::endl;
        return *this;
    }
    ...
    bool isAtEnd() const {
        return static_cast<int>(bucket_index) >= (1 << set->directory.global_depth);
    }
void skip() {
    size_type size = 1 << set->directory.global_depth;
    //std::cerr << "Starting skip: bucket_index=" << bucket_index << ", element_index=" << element_index << std::endl;
    
    //if (!visited) return;
    
    while (bucket_index < size) {
        /* std::cerr << " Inspecting bucket_index=" << bucket_index
                  << ", element_index=" << element_index
                  //<< ", visited=" << visited[bucket_index]
                  << ", bucket count=" << set->directory.buckets[bucket_index]->count << std::endl; */
...
/*             for (size_t j{bucket_index}; j < size; j += (1 << set->directory.buckets[bucket_index]->local_depth)) {
                if(set->directory.buckets[bucket_index] == set->directory.buckets[j]){
                    visited[j] = true;
                std::cerr << "------------\n Marking bucket_index=" << j << " as visited \n ------------";
                }
            } */
            
            ++bucket_index;
            element_index = 0;
            /* while(bucket_index < size && set->directory.buckets[bucket_index] == set->directory.buckets[bucket_index -1]){
                ++bucket_index;
            } */
        } else {
            bool lowest = true;
            size_t step = 1 << set->directory.buckets[bucket_index]->local_depth;
            for(size_t i{bucket_index % step}; i < bucket_index; i+= step){
                if(set->directory.buckets[i] == set->directory.buckets[bucket_index]){
                    lowest = false;
                    break;
                }
            }
    ...
    // Correctly handle the end condition
    if (bucket_index >= size) {
        bucket_index = size; // Make sure we don't access beyond valid directory size
        element_index = 0;
    }
    
    //std::cerr << "Ending skip: bucket_index=" << bucket_index << ", element_index=" << element_index << std::endl;
}
};
