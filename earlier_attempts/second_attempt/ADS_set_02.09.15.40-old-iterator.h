...
std::pair<typename ADS_set<Key, N>::iterator,bool> ADS_set<Key, N>::insert(const typename ADS_set<Key, N>::key_type &key) {
    ...
    return {iterator(this, hash, (directory.buckets[hash]->count)-1), true};
}
...
size_t get_ele() const{return element_index;}
  ...
  Iterator() : set(nullptr), visited(nullptr), bucket_index(0), element_index(0), size(0) {}
  ...
  Iterator(const ADS_set* set, size_t bucket_index = 0, size_t element_index = 0) : set(set), bucket_index(bucket_index), element_index(element_index) {
    size = 1 << set->directory.global_depth;
    visited = new bool[size]{false};
    if (bucket_index < size && set->directory.buckets[bucket_index]->count == 0) {skip();}
  };
Iterator(const Iterator& other) : set(other.set), bucket_index(other.bucket_index), element_index(other.element_index) {
...
private:
    ...
    void skip() {
        size_t size = 1 << set->directory.global_depth;
        while (bucket_index < size) {
            if (element_index >= set->directory.buckets[bucket_index]->count) {
                element_index = 0;
                ++bucket_index;
            } else {
                current_pointer = &(set->directory.buckets[bucket_index]->elements[element_index]);
                return;
            }
        }
...
public:
    Iterator() : set(nullptr), bucket_index(0), element_index(0), current_pointer(nullptr) {}
    Iterator(const ADS_set* set, size_t bucket_index = 0, size_t element_index = 0)
        : set(set), bucket_index(bucket_index), element_index(element_index), current_pointer(nullptr) {
        skip();
    }
    size_t get_buck() const{return bucket_index;}
    size_t get_ele() const{return element_index;}
    ...
    friend bool operator==(const Iterator &lhs, const Iterator &rhs) {
        return lhs.current_pointer == rhs.current_pointer;
    }
    friend bool operator!=(const Iterator &lhs, const Iterator &rhs) {
        return !(lhs == rhs);
    }
