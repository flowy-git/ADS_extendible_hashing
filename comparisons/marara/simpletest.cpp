 ...
 #include <random>
 #include <numeric>
 #include "ADS_set_marara_copy.hpp"
 #include <type_traits>
 #include <chrono>
 ...
 int main() {
  auto start = std::chrono::high_resolution_clock::now();
   ...
   /* auto c_begin = const_c->begin();
auto c_end = const_c->end();

if (c_begin != c_end) {
    std::cout << "Current iterator position: " << *c_begin << std::endl;
}*/

    for (auto c_it {const_c->begin()}; !(c_it == const_c->end());) {
        std::cout << "Current iterator position: " << *c_it << std::endl;
        v.push_back(*c_it++.operator->());
    }

    if (v.size() != r->size()) {
        std::cout << "ERROR - vector fill failed (wrong size)" << v.size() << " vs " << r->size() << std::endl;
        break;
    }

    if (!std::is_permutation(v.begin(), v.end(), r->begin(), std::equal_to<Key>{})) {
        std::cout << "ERROR - vector fill failed (duplicate/missing elements)" << std::endl;
        break;
    }
    ...
    for (auto v_it {v.begin()}; v_it != v.end(); ++v_it) {
        ...
        if (!std::equal_to<Key>{}(*c_it, *v_it)) {
            std::cout << "ERROR - iterator returned by find does not match" << std::endl;
            std::cout << "Expected: " << *v_it << " Found: " << *c_it << std::endl;
            break;
        }

        if (!std::equal(v.begin(), v_it, const_c->begin(), c_it, std::equal_to<Key>{}) || !std::equal(v_it, v.end(), c_it, const_c->end(), std::equal_to<Key>{})) {
            std::cout << "ERROR - ranges do not match" << std::endl;
            std::cout << "Expected range: [" << *v.begin() << " - " << *v_it << "] Found range: [" << *const_c->begin() << " - " << *c_it << "]" << std::endl;
            std::cout << "Expected range: [" << *v_it << " - " << *v.end() << "] Found range: [" << *c_it << " - " << *const_c->end() << "]" << std::endl;
            break;
        }
    }

    std::cout << "Iterator test passed successfully" << std::endl;
    break;
}
 #endif // PH2
         default:
