...
for (const Key &k: key_v) test_erase(k, c, r, true);
...
for (const Key &k: key_v) test_find(k, const_c, r, true);

} else {

for (const auto &k: *r) test_find(k, const_c, r);
...
// Populating the vector `v` with elements from `const_c`

for (auto c_it {const_c->begin()}; c_it != const_c->end();) {

std::cerr << "-------------------------------Adding element " << *c_it << " to vector\n";

v.push_back(*c_it++);

}

// Log the contents of vector `v`
...
// Log current state before range checks

std::cerr << " ---------------------\n Checking element: " << *v_it << "\n--Set version: " << *c_it << std::endl;

std::cerr << "Position in vector: " << std::distance(v.begin(), v_it) << std::endl;

// Add protection to detect end state

size_t dist = (c_it != const_c->end()) ? std::distance(const_c->begin(), c_it) : -1;

std::cerr << "Position in set: " << dist << std::endl;

if (c_it == const_c->end() || !std::equal_to<Key>{}(*c_it, *v_it)) {

std::cerr << " ERROR - iterator returned by find - " << *c_it << " vs " << *v_it << std::endl;

break;

}

// Check and output before range check

bool pre_check1 = std::equal(v.begin(), v_it, const_c->begin(), c_it, std::equal_to<Key>{});

bool pre_check2 = std::equal(v_it, v.end(), c_it, const_c->end(), std::equal_to<Key>{});

if (!pre_check1 || !pre_check2) {

std::cerr << " ERROR - ranges" << std::endl;

// Detailed range output

std::cerr << "Mismatched ranges:" << std::endl;

std::cerr << "Vector (begin to current): ";

for (auto it = v.begin(); it != v_it; ++it) {
...
std::cerr << "Set (begin to current): ";

for (auto it = const_c->begin(); it != c_it; ++it) {
...
std::cerr << "Vector (current to end): ";

for (auto it = v_it; it != v.end(); ++it) {
...
std::cerr << "Set (current to end): ";

for (auto it = c_it; it != const_c->end(); ++it) {
...
std::cerr << "Iterator test passed lol";
}
