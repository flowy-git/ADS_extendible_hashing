...
void test_copy(ads::set<val_t> const& a, std::set<val_t> const& r) {
    ...
    std::cout << "\n=== checkpoint4 ===\n";
    std::cerr << "Initialized copy_a and copy_r\n";
    std::cout << "\n=== checkpoint5 ===\n";

    std::cerr << "Original size: " << a.size() << ", Copy size: " << copy_a.size() << '\n';
    std::cout << "\n=== checkpoint6 ===\n";
    ...
    std::cout << "\n=== checkpoint10 ===\n";
    std::cerr << "Completed count comparison of all elements in original set\n";
    ...
    std::cerr << "Equality check passed\n";

    for (size_t i = 0; copy_a.size() == r.size(); ++i) {
        std::cerr << "in (copy) " << i << '\n';
        copy_a.insert(i);
        copy_r.insert(i);
        std::cerr << "Inserted " << i << " into copy_a and copy_r\n";
    }
    std::cerr << "Completed insertion loop\n";
    ...
    std::cerr << "Completed count consistency check between original and copy_r\n";
    ...
    std::cerr << "Final equality check passed\n";

    sanity_check("test_copy", copy_a, copy_r);
    std::cerr << "Completed sanity check\n";
}
